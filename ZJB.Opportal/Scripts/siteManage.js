...
$(function(){
	...
	$(".site_check_function").die().live("click",function(){
		...
		art.dialog.confirm('检测绑定的账号密码并激活成功账号?', function(){
			$.ajax({ 
		        type: "post", 
		        url: contextPath + "/ajax/site/check.do", 
...
function loginWeb(that){
	...
	var url = '/site/manage.do?userWebId=' + userWebId;
	window.open(url);
}
