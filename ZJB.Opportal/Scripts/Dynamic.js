function LybHandler(param) {
    ...
    this.SetTopBtn = jQuery("#btnSetTopOk");
    this.Init();
}
LybHandler.prototype = {
    ...
    FormateLyb: function (n) {
        var me = this;
        var bulider = new StringBulider();
        bulider.append('<div class=\"twt-type- mb-twt msg kcmp tsq_zt_bs\" style=\"margin:0px\" data-id="', n.Id, '" data-username="', n.UserName, '" data-userId="', n.UserId, '" data-lastCommentId="0">');
        var icon = n.Portrait == "" ? me.defaultIcon : n.Portrait;
        bulider.append('				<a href="javascript:;" class="namecard avatarbig"><img alt="头像" UserName="this.src=\'', me.defaultIcon, '\'" src="', icon, '?imageView2/1/w/50/h/50/q/85" class="avatar"></a>');
        bulider.append('				<div class="msg-bd">' );
        bulider.append('				<div class="msg-main">');
        if (n.IsTop == 1) {
            bulider.append('<span class="setTopIco">置顶</span>');
        }
        bulider.append('<a href="javascript:;" class="mb-twt-username namecard">', n.UserName, '</a>');
        bulider.append('<br/>');
        var noWrapContent = n.DynamicContent.replace(/<\/?.+?>/g, "");//WrapContent.replace(/<\/?.+?>/g, "");
        ...
        var content = me.AddAUserHtml(n.DynamicContent, n.RemindUsers, remindUserHtml); // n.WrapContent + (remindUserHtml.length > 0 ? " " + remindUserHtml.join(" ") : "");
    ...
    FormateImage: function (pics) {
        ...
        for (var i = 0; i < length; i++) {
            var picObj = arr[i];
            var pic = picObj.ImageUrl + "?imageView2/2/w/100/h/100"
    ...
    FormateBtn: function (n) {
      ...
      // var collectionBtnTxt = n.IsCollection ? "取消收藏" : "收藏";
        ...
        var topBtnTxt = n.IsTop == 1 ? "取消置顶" : "置顶";
        bulider.append('<a class="msg-ctrl-settop"  lybid="',n.Id,'" type="', n.IsTop, '">', topBtnTxt, '</a>');
    ...
    BindBtnEvent: function () {
        ...
        tsq_lyb_dbcd.find(".msg-ctrl-settop").off("click").on("click", function () {
            me.SetTopToShow(this);
        });
    ...
    , SetTopToShow: function (obj) {
        var me = this;
        var obj = jQuery(obj);
        var lybid = obj.attr("lybid");
        var type = obj.attr("type");//1是已置顶 0是未置顶
        var offset = obj.offset();
        jQuery("#settoplybid").val(lybid);
        if (type == 0) {
            jQuery("#setTopDiv_lyb").css({ top: ((offset.top) + "px"), left: ((offset.left) + "px") }).show();
        }
        else {
            if (confirm("确定取消置顶"))
            {
    ...
    , SaveSetTopDays: function (otype) {
        var me = this;
        var otype = otype == undefined ? 0 : otype; //0 表示置顶 1 表示取消置顶
        var flag = false;
        var days = 0;

        jQuery.each(jQuery("#setTopDiv_lyb").find("input[name='settopdays']"), function (i, obj) {
            if (jQuery(obj).is(":checked")) {
                flag = true;
                days = jQuery(obj).val();
            }
        });
        if (!flag && otype == 0) {alert('请选择要置顶的天数'); return false; }
        else {
            var lybid = jQuery("#settoplybid").val();
            var url = "/Social/DynamicSetTop";
            jQuery.post(url, {days: days, lybid: lybid, otype: otype },
                function (result) {
                    var msg = result.status;
                    if (msg>0) {
                        var checked = jQuery("#setTopDiv_lyb").find("input[name='settopdays']").eq(0);
                        jQuery(checked).attr('checked', true);
                        jQuery("#setTopDiv_lyb").hide();
                        alert(otype == 0 ? '置顶成功！' : '取消置顶成功！');
                        me.InitValue();
                        me.GetList();
                    }
                });
...
ReplyHandler.prototype = {
    ...
    FormateReplyRecord: function (obj, ismore) {
        ...
        jQuery.each(obj.CommentList, function (i, n) {
            ...
            bulider.append('<a href="javascript:Global.LinkUserMain(', n.UserId, ')" class="namecard"><img  alt="头像" src="', pic, '?imageView2/1/w/30/h/30/q/85" class="avatar"></a>');
            bulider.append('<div class="msg-bd">');
            bulider.append('<div class="msg-main">');
            ...
            bulider.append('<span>', n.DynamicContent, '</span>');
            bulider.append('</div></div>');
            bulider.append('<div class="msg-ft">');
            bulider.append('<div class="msg-ctrl">');
            bulider.append('<a  class="msg-ctrl-cmt" data-id="', n.Id, '">回复</a>');
            bulider.append('<a  onclick="ShowUserPoint(' + n.UserId + ')" data-id="', n.UserId, '">积分</a>');
