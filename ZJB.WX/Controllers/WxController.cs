...
{
    public partial class WxController : Controller
    {
        ...
        public ActionResult Get(PostModel postModel, string echostr)
        {
            ...
            {
                return Content("failed:" + postModel.Signature + "," + Senparc.Weixin.MP.CheckSignature.GetSignature(postModel.Timestamp, postModel.Nonce,WeixinConfig.WeixinDic.FirstOrDefault().Value.AppToken) + "。" +
        ...
        public ActionResult Post(PostModel postModel)
        {
            ...
            postModel.Token = weixinInfo.AppToken;//根据自己后台的设置保持一致
            ...
            catch (Exception ex)
            {
                ...
                {
                    tw.WriteLine("APPID:" + weixinInfo.AppId);
        ...
        #region 获取公众号信息
        private WeixinInfo GetWeixinApp(string signature, PostModel postModel)
        {
            return GetWeixinApp(postModel.Signature, postModel.Timestamp, postModel.Nonce);
        }
        private WeixinInfo GetWeixinApp(string signature, string timestamp, string nonce)
        {
           // Logger logger = new Logger("ExceptionLog");
           // logger.Debug(string.Format("signature:{0},timestamp:{1},nonce:{2}", signature, timestamp, nonce));
          //  logger.Debug(WeixinConfig.WeixinDic.ToJson());
            foreach (var item in WeixinConfig.WeixinDic)
            {
                //logger.Debug(" item.Value.AppToken:" + item.Value.AppToken);
                if (CheckSignature.Check(signature, timestamp, nonce, item.Value.AppToken))
                {
                    //logger.Debug("命中 item.Value.AppId:" + item.Value.AppId);
                    return item.Value;
                }
            }
            return null;
