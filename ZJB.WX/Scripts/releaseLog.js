...
function BuildLogHtml(datas) {
    ...
    if (data != null && data.length > 0) {
        $.each(data, function (a, b) {
            ...
            var btnReReleaseHtml = "";
            if (b.ReRelease == 1)
            {
                btnReReleaseHtml = "<br/><a href='javascript:void(0)' onclick='LogReRelease("+b.ID+","+b.Status+")'>重发</a>";
            }
            html += '				<tr id="htr' + b.ID + '">' +
                    '					<td width="60">' +
...
////发布日志重发
function LogReRelease(id,status)
{
    $.ajax({
        type: 'post',
        dataType: 'json',
        url: '/PostLog/ReReleaseLog',
