...
{
    if (checkTel() && checkName()&&isCanSubmit_Tel && isCanSubmit_Name  &&CheckPwd() && checkCity())
    {
        var tel = $("#tel").val();
        var name = $("#name").val();
        var pwd = $("#userPwd").val();
...
function UserUpdatePwd() {
    if (checkTel() && isCanSubmit_Tel &&  CheckPwd() ) {
        var tel = $("#tel").val();
        var pwd = $("#userPwd").val();
        var vaildCode = $("#code").val();
        $.ajax({
            ...
            data: { Tel: tel, Password: pwd, code: vaildCode, invitationCode: $("#InvitationCode").val() },
            success: function (result) {
                art.dialog.alert(result.msg);
                if (result.status == 0) {
                    setTimeout(function () { window.location.href = "/Home"; }, 1000);
...
function InviteRegister() {
    if (CheckPwd() ) {
        ...
        $.ajax({
            type: 'post',
            dataType: 'json',
            url: '/User/InviteRegister',
            data: { Tel: tel, Name: name, Password: pwd, CityID: cityid, info: info, from: from },
            success: function (result) {
                art.dialog.alert(result.msg);
                if (result.status == 0) {
                    setTimeout(function () { window.location.href = "/Home"; }, 1000);
        ...
        $.ajax({
            ...
            success: function (result) {
                if (result.status == 1) {
                    ...
                    art.dialog.alert("刚刚已发送验证码，请查收短信。");
                }
            }
        });
    }
